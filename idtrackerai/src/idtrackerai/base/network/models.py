...
class IdCNN(nn.Module):
    ...
    def __init__(self, input_shape: Sequence[int], out_dim: int):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Flatten(),
            # SLEAP-PORT: generalised to non-square identification images.
            # Original:
            #     nn.Linear(100 * (input_shape[1] // 4) ** 2, 100),
            # The square assumed height == width, which held upstream only
            # because Session.set_id_image_size auto-derives [max_size,
            # max_size, 1] (session.py:543). This port sizes id-images from the
            # population median of the animal's x-spread and y-extent
            # separately, which for flies is 57 x 83 -- so the flatten must be
            # H//4 * W//4, not (W//4)^2. Exactly equivalent whenever
            # input_shape[0] == input_shape[1], so square models and their
