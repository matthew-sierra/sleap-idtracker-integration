...
Thus, this project aims to integrate SLEAP and idTracker.ai together to produce high quality pose estimation alongside high quality identity tracking. The remainder of this document will describe the architecture used by the model, as well as provide an overview of how each of the individual modules. We will give a brief introduction of idTracker.ai and SLEAP, followed by a description of the changes that we have implemented in this project.
...
## Overview of the idTracker.ai architecture

The architecture for this project is based extensively on the idTracker.ai architecture, so we will provide an overview of how it functions to then explain what changes we have made in our integration.

Generally, idTracker.ai has the following workflow:

1. Thresholding (manually done by user before any processing is done)
2. Create chunks called episodes (used so that the model could process data in parallel)
3. "Blob" creation where we use thresholding information to separate individuals from the background
4. Create square images of uniform size that contain the blobs oriented similarly to each other
5. Create an HDF5 file that contains all of the training examples
6. Generate graphs by using information from the crossings where we can say an individual has certainly kept the same identity
7. Generate fragments where we can clearly track individuals
8. Generate batches for the model using the fragments that we have created. It only uses fragments where it is sure that individuals are separate from one another
9. Train the contrastive learning model
10. Predict on all the instances by iterating over the HDF5 file
11. Cluster the representation space using k-means clustering
12. Generate frame-by-frame trajectories for each individual using identities formed in steps 10 and 11 that the user is able to access in the idTracker.ai GUI.

The biggest change that we hoped to make in our implementation is cutting out the thresholding and blob steps, since we are already able to identity all individuals that appear on a screen thanks to SLEAP's predictions.
## SLEAP Architecture Key Features

We also take some concepts from the SLEAP architecture as we create this implementation, so we briefly describe them here. Obviously, we are using the instances that were generated by SLEAP. Each of the instances have a global index within the SLEAP framework, which we use when we assign individual indexes in the implementation.  We are also using the same methods for creating the bounding boxes that are defined in the SLEAP architecture, so we end up with identical bounding boxes.

The way that SLEAP attempts to create matches in between frames is that it computes a cost matrix for all the instances from one frame to the next. The entries of this matrix are the keypoint, bounding box, or centroid similiarity between all of the instances. SLEAP is only able to match instances between two frames if it is able to track one individual from one frame to the next: *it never chooses to make a new track based on whether or not it had a low score.* Therefore, whenever we have new tracks generated, it is only because it saw that there was one frame in between where there was no match. We use the same cost matrix, but a modified matching method, to determine whether or not two instances are matching.
